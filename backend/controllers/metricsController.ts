...
export async function getMetrics(req: Request, res: Response) {
  try {
    const [avgResult, flagged, total, low, medium, high] = await Promise.all([
      (await Claim.findOne({
        attributes: [
          [
      ...
      await Claim.count(),
      Claim.count({ where: { fraud_category: "Low" } }),
      Claim.count({ where: { fraud_category: "Medium" } }),
      Claim.count({ where: { fraud_category: "High" } }),
    ]);
